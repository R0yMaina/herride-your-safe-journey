...
export function AppHeader({ title, back = "/home", onBack, sos = true }: { title?: string; back?: string; onBack?: () => void; sos?: boolean }) {
  const BackButton = () => (
  ...
  return (
    <div className="sticky top-0 z-20 px-4 pt-4 pb-3 bg-background/80 backdrop-blur-xl flex items-center justify-between">
      {onBack ? (
        <button onClick={onBack} type="button" className="outline-none">
          <BackButton />
        </button>
      ) : (
        <Link to={back as any}>
