...
export function Display({ className, ...p }: HTMLAttributes<HTMLHeadingElement>) {
  return <h1 className={cn("font-display text-4xl leading-tight text-foreground", className)} {...p} />;
}
...
export function Eyebrow({ className, ...p }: HTMLAttributes<HTMLSpanElement>) {
  return <span className={cn("text-[10px] font-medium uppercase tracking-[0.28em] text-primary/80", className)} {...p} />;
}