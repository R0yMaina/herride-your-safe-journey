...
import { ApiError, type ApiResponse, type RequestInterceptor, type RequestOptions, type ResponseInterceptor } from "./types";
...
class ApiClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<ApiResponse<T>> {
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
      try { return await this.dispatch<T>(path, options); }
      catch (error) {
        lastError = error;
  ...
  private async dispatch<T>(path: string, options: RequestOptions): Promise<ApiResponse<T>> {
    ...
    const timeout = setTimeout(() => controller.abort(), options.timeoutMs ?? appConfig.api.timeoutMs);
    ...
    for (const interceptor of this.requestInterceptors) ({ url, init } = await interceptor(url, init));
    try {
      let response = await fetch(url, init);
      for (const interceptor of this.responseInterceptors) response = await interceptor(response);
      if (!response.ok) throw new ApiError(response.statusText, response.status, `http_${response.status}`);
